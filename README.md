...
- 每个人格有 3 个独立目的地，共 18 个候选，覆盖亚洲、欧洲、北美、非洲和大洋洲
- 同一人格下会根据预算、美食、文化、自然、舒适度和短途倾向继续细分，不再纯随机推荐
- 完全本地运行，不需要 API Key，也不会发起 AI 或外部模型请求
- 结果卡适合截图分享，每个人格配有专属头像形象
- 结果页背景、推荐文案和 Skyscanner CTA 会随匹配目的地同步变化，并直达该地酒店或从访问地区出发的航班页面
- 一键复制分享文案和好友匹配链接
- 好友打开匹配链接后答题，结果卡会显示旅行搭子 match 百分比
- 一键生成 PNG 结果卡，移动端优先调起系统保存/分享面板
- 分享图片内置好友匹配二维码，扫码即可打开对应的匹配测试
...
二维码由项目内置的 `qrcode-generator 1.4.4` 在浏览器本地生成，匹配链接不会发送给第三方二维码服务；许可证见 `vendor/LICENSE.qrcode-generator.txt`。
...
结果会先确定主旅行人格，再用全部答案中的次级偏好给该人格下的三个目的地加权。例如同为“美食雷达”，偏文化会更容易匹配墨西哥城，偏舒适与慢节奏会更容易匹配博洛尼亚。
