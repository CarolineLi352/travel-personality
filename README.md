...
- 自动加权计算旅行人格类型
- Lite AI 个性化题目：保留固定分支和计分，AI 会根据前面选择改写下一题和选项文案
...
答题过程中，页面会先显示内置题库；每答完一题后，会尝试调用 `/api/personalize-question` 微调下一题和选项文案。答完题后，页面会先显示默认结果卡，然后调用 `/api/generate-result` 生成更个性化的结果。如果没有配置 `OPENAI_API_KEY`，会自动保留内置题库和默认结果卡。

AI 只负责改写文案，不负责改变选项数量、分支、计分或好友匹配逻辑，所以测试结果仍然稳定可复现。
